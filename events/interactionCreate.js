        ...
        if (interaction.isChatInputCommand()) { // ensure only slash commands
            ...
            if (interaction.customId == 'betModal') {
                ...
                const opt1Btn = new ButtonBuilder()
                    .setCustomId(`opt1Btn_${scenario}`) // attach senario to buttons so we can extract it later
                    .setLabel(`1`)
            ...
            if (!activeReactionArray.includes(interaction.user.id)) {
                ...
                activeReactionArray.push(interaction.user.id);
                ...
                let scenario = '!! ERROR !!';
                if (selectedBtn == 'opt1Btn' || selectedBtn == 'opt2Btn') {
                ...
                if (selectedBtn != 'finishBtn') {
                    await interaction.reply({ content: `<@${interactionUser.id}> Enter amount you want to wager`, ephemeral: true});
                    const messageFilter = m => m.author.id === interactionUser.id;
                    const collector = interaction.channel.createMessageCollector({ filter: messageFilter, max: 1, time: 10000 });
                    collector.on('collect', m => {
                        ...
                        if (userInput > max_wager) {
                            interaction.followUp({ content: `Bet amount exceeds available balance`, ephemeral: true });
                            return;
                        }
                        interaction.followUp({ content: `${interactionUser.nickname} has wagered ${m} Slab Bucks on **${scenario}**` });
                        messageReceived = 1;

                    });

                    collector.on('end', m => {
                        const index = activeReactionArray.indexOf(interactionUser.id);
                        activeReactionArray.splice(index, 1);
                        if (!messageReceived) {
                            interaction.editReply({ content: `Time's up!`, ephemeral: true});
                        }
                    });
                ...
                await interaction.reply({ content: 'Cannot proceed: Awaiting previous event', ephemeral: true });
            }
        }
    }
};
